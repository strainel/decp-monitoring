...
echo "date,aife_dume,dgfip_pes,emarchespublics,grandlyon,marchespublicsinfo,atexomaximilien,ternumbfc,megalisbretagne,regionbretagne" > ${DIROUT}/${FILEOUT}
jq --raw-output '[.date, .sources."data.gouv.fr_aife", .sources."data.gouv.fr_pes" , .sources."e-marchespublics", .sources."grandlyon", .sources."marches-publics.info", .sources."atexo-maximilien", .sources."ternum-bfc", .sources."megalis-bretagne", .sources."region-bretagne"] | @csv' ${DIROUT}/*.jq >> ${DIROUT}/${FILEOUT}
